...
A player who fails to reach the target size before time expires sees a defeat screen and understands they have failed the level.

**Why this priority**: Failure conditions are essential for challenge and meaningful gameplay. Without time pressure and consequences, there's no urgency or tension.

**Independent Test**: Can be tested by starting a level and intentionally letting the timer expire without reaching target size. Verifies defeat feedback works independently of victory flow.
...
1. **Given** player starts a level, **When** gameplay begins, **Then** timer displays current time remaining and counts down each second
2. **Given** player is playing any level, **When** timer is running low (e.g., <10 seconds), **Then** timer provides visual urgency feedback (e.g., color change, pulse animation)
...
1. **Given** Level 1 starts (small size range 0.5-5.0), **When** collectibles spawn, **Then** collectibles range from 0.3 to 3.0 in size
2. **Given** Level 2 starts (medium size range 5.0-15.0), **When** collectibles spawn, **Then** collectibles range from 3.0 to 10.0 in size
3. **Given** Level 3 starts (large size range 15.0-50.0), **When** collectibles spawn, **Then** collectibles range from 10.0 to 40.0 in size
...
1. **Given** Level 1 configuration, **When** level parameters are checked, **Then** target size is 5.0, timer is 60 seconds, difficulty is "Easy"
2. **Given** Level 2 configuration, **When** level parameters are checked, **Then** target size is 15.0, timer is 90 seconds, difficulty is "Medium"
3. **Given** Level 3 configuration, **When** level parameters are checked, **Then** target size is 50.0, timer is 120 seconds, difficulty is "Hard"
...
1. **Given** player completes a level, **When** victory condition triggers, **Then** victory screen displays for 2-3 seconds with level completion stats (final size, time remaining)
2. **Given** player fails a level, **When** defeat condition triggers, **Then** defeat screen displays for 2-3 seconds with failure reason and encouragement to retry
3. **Given** transition screen is showing, **When** display time elapses, **Then** automatic progression to next game state occurs (next level or game over)
...
- **FR-001**: System MUST support configuration for 3 distinct levels with unique parameters (target size, time limit, collectible size ranges)
- **FR-002**: System MUST display countdown timer starting from level-specific time limit and decrementing each second
...
- **FR-007**: System MUST automatically progress to next level after victory screen display period
- **FR-008**: System MUST clear all collectibles from previous level when new level starts
- **FR-009**: System MUST spawn fresh collectibles appropriate to current level size range when level begins
- **FR-010**: System MUST display current size and target size in HUD throughout gameplay
- **FR-011**: System MUST reset player size to starting size when new level begins
- **FR-012**: System MUST pause timer when game is paused [NEEDS CLARIFICATION: game pause functionality may not exist yet]
- **FR-013**: System MUST handle Level 3 completion by showing final victory/game complete screen

### Key Entities *(include if feature involves data)*

- **Level Configuration**: Defines parameters for each level including level number (1-3), target size (goal player must reach), time limit (seconds), starting player size (typically 0.5), collectible size range (min/max for spawning), collectible spawn count, difficulty label (Easy/Medium/Hard)
...
- **Transition Screen**: Represents victory or defeat screen state including screen type (victory/defeat), display duration (2-3 seconds), level statistics (final size, time remaining/expired), next action (proceed to next level, retry, game complete)
...
- **SC-005**: Victory and defeat screens display for 2-3 seconds with clear feedback before transitioning
